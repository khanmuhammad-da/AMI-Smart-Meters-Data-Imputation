...
def render_markdown(content: str, unsafe_allow_html: bool = True):
    """Render Markdown normally and raw HTML with st.html.

    Streamlit's Markdown parser can display multiline HTML as a code block in
    some layouts/versions. Raw HTML/CSS is therefore sent directly to st.html.
    Plain Markdown is still sent to st.markdown.
    """
    content = textwrap.dedent(content).strip()

    # Use Streamlit's dedicated HTML renderer for actual HTML/CSS.
    if unsafe_allow_html and "<" in content and ">" in content:
        if hasattr(st, "html"):
            st.html(content)
        else:
...
left_logo = (
    f'<img class="brand-logo-left" src="{ngcp_uri}" alt="NGCP logo" style="width:88px;height:88px;object-fit:contain;display:block;">'
    if ngcp_uri
    else ''
)

right_logo = (
    f'<img class="brand-logo-right" src="{giki_uri}" alt="GIKI logo" style="width:88px;height:88px;object-fit:contain;display:block;">'
    if giki_uri
    else ''
)
...
with st.sidebar:
    render_markdown("### ⚡ Inference Controls")
...
if run_button:
    ...
    try:
        ...
        render_markdown('<div class="section-note">Monthly energy reconciliation and missing-load diagnostics</div>', unsafe_allow_html=True)
...
""",
        ...
        render_markdown('<div class="section-note">Complete 30-minute timeline from the raw meter grid</div>', unsafe_allow_html=True)
        ...
        render_markdown('<div class="section-note">Comparison of ML-reconciled energy against the office mean baseline</div>', unsafe_allow_html=True)
        ...
        render_markdown('<div class="section-note">Export the reconciled dataset and comparison visualization</div>', unsafe_allow_html=True)
